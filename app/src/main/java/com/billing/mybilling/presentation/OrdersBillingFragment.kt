...
class OrdersBillingFragment : BaseFragment<FragmentOrdersBillingBinding, HomeViewModel>() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        homeViewModel.totalAmount.observe(viewLifecycleOwner) {
            ...
            getViewDataBinding().deliveryCharge.text =
                homeViewModel.pendingOrders?.delivery_charges.toString().setPrice()
        ...
        getViewDataBinding().imgQrCode.setImage(sessionManager.getUser()?.business_payment_qr_code)
    }

    fun addDiscount() {
        findNavController().navigate(
            OrdersBillingFragmentDirections.actionOrdersBillingFragmentToAddDiscountFormFragment(
                editType.DISCOUNT.type,
                Gson().toJson(homeViewModel.pendingOrders)
            )
        )
    }
    ...
    fun handleNavigation(){
        sessionManager.getUser()?.let {
            if (it.user_type_id == UserType.STAFF.id){
                findNavController().navigate(OrdersBillingFragmentDirections.actionOrdersBillingFragmentToPendingOrders())
            }else{
    ...
    private fun shareOnWhatsApp() {
        ...
        try {
            ...
            //Custom Font
            val fontName =
                BaseFont.createFont("res/font/poppins_regular.ttf", "UTF-8", BaseFont.EMBEDDED)
            val topTitleFontName =
                BaseFont.createFont("res/font/grillmedium.otf", "UTF-8", BaseFont.EMBEDDED)

            val packageName = "com.billing.mybilling"
            val uri =
                FileProvider.getUriForFile(requireContext(), "${packageName}.provider", filePath)
...
//            image.alignment = Image.MIDDLE
            ...
            val topTitleFont = Font(topTitleFontName, 22.0f, Font.NORMAL, BaseColor.BLACK)
            addNewItem(document, "${user?.business_name}\n${user?.business_address}\n${user?.business_whats_app_no}", Element.ALIGN_CENTER, topTitleFont)
            ...
            model?.products?.forEach { products ->
                ...
                addNewItemWithLeftAndRight(
                    document,
                    "$counter. " + products.product_name,
                    "Quan :  ${products.selected_quan} X Rs. $price",
                    titleFont,
                    orderNumberValueFont
                )
                addNewItemWithLeftAndRight(
                    document,
                    "    " + products.variant_name.toString(),
                    "Rs. ${finalPrice}",
                    titleFont,
                    orderNumberValueFont
                )
...
//            val finalPrice = homeViewModel.finalAmount.toInt() - discount
            ...
            addNewItem(
                document,
                "** Thank you so much for visiting ${user?.business_name} **",
                Element.ALIGN_RIGHT,
                orderThankYouFont
            )
            document.close()
